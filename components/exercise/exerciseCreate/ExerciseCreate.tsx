...
type ExerciseCreateProps = {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const exerciseConfigs = await fetchExerciseConfigsForMuscleGroup(muscleId)
      ...
      <SafeAreaView>
        <View style={{flexDirection: 'row', flexWrap: 'wrap'}}>          
          ...
          {return <View key={exercise.id} >
            <Pressable onPress={() => { addExerciseForWorkout(workoutId, exercise.id); (navigation as any).navigate('WorkoutPage', {
      workoutId: workoutId,
    })}}>
